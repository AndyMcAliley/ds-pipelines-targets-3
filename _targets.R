...
list(
  ...
  # group oldest_active_sites by state
  ...
  tar_target(timeseries_png,
             plot_site_data(sprintf("3_visualize/out/timeseries_%s.png", unique(nwis_data$State)),
                            nwis_data,
                            parameter),
             format = "file",
  ...
  tar_target(summary_state_timeseries_csv, 
             summarize_targets('3_visualize/log/summary_state_timeseries.csv', names(timeseries_png)), 
             format="file"),
  # Plot data coverage
